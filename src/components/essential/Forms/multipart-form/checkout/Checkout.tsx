...
import { matchingArrayElements } from '@/components/actions/action';
import {
	L2_RequitedField,
	L3_RequitedField,
	payment,
...
function Checkout({
	...
	const allData = {
		...
		start_date: state.level2.start_date,
		end_date: state.level2.end_date,
		audience: 'no data',
		...
		const cb = async () => {
			...
			try {
				...
				if (dataA.data === 'success') {
					window.location.href = dataA?.message?.payment_url
			...
			<div className={style.orderBox}>
				...
				<div className={style.summeryBox}>
					<SummeryRow
						...
						text={{ h: 'Total Budget', p: 'USD Currency' }}
					/>
					<SummeryRow pp={`৳ ${dollar}`} text={{ h: 'Dollar Rate' }} />
					<SummeryRow
						pp={`৳ ${parseInt(state.level2.budget_amount) * parseInt(dollar)}`}
